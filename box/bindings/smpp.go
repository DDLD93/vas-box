...
func DoProcess(senderId string, recievers []string)  {
	func sendSMS() {
		 wg:= sync.WaitGroup
	...
	func newSubmitSM() *pdu.SubmitSM {
		...
		// srcAddr.SetTon(5)
		// srcAddr.SetNpi(0)
		_ = srcAddr.SetAddress(senderId)
	
		destAddr := pdu.NewAddress()
		// destAddr.SetTon(1)
		// destAddr.SetNpi(1)
		_ = destAddr.SetAddress("2347055793353")
	
		submitSM := pdu.NewSubmitSM().(*pdu.SubmitSM)
		submitSM.SourceAddr = srcAddr
		submitSM.DestAddr = destAddr
		_ = submitSM.Message.SetMessageWithEncoding("GOLAND SMPP TEST", data.UCS2)
